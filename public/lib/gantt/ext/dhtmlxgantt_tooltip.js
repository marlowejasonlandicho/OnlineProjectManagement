/*
 ...
 (c) Dinamenta, UAB.
 */
gantt._tooltip = {}, gantt._tooltip_class = "gantt_tooltip", gantt.config.tooltip_timeout = 30, gantt.config.tooltip_offset_y = 20, gantt.config.tooltip_offset_x = 10,
        gantt._create_tooltip = function () {
            return this._tooltip_html || (this._tooltip_html = document.createElement("div"),
                    this._tooltip_html.className = gantt._tooltip_class, this._waiAria.tooltipAttr(this._tooltip_html)),
                    this._tooltip_html
        },
        ...
        gantt._show_tooltip = function (t, e) {
            if (!gantt.config.touch || gantt.config.touch_tooltip) {
                ...
                var a = n.offsetWidth + 20, i = n.offsetHeight + 40, s = this.$task.offsetHeight, r = this.$task.offsetWidth, o = this.getScrollState();
                gantt._waiAria.tooltipVisibleAttr(n), e.y += o.y;
                var l = {x: e.x, y: e.y};
                e.x += 1 * gantt.config.tooltip_offset_x || 0, e.y += 1 * gantt.config.tooltip_offset_y || 0, e.y = Math.min(Math.max(o.y, e.y), o.y + s - i), e.x = Math.min(Math.max(o.x, e.x), o.x + r - a),
                        gantt._is_cursor_under_tooltip(l, {pos: e, width: a, height: i}) && (l.x + a > r + o.x && (e.x = l.x - (a - 20) - (1 * gantt.config.tooltip_offset_x || 0)), l.y + i > s + o.y && (e.y = l.y - (i - 40) - (1 * gantt.config.tooltip_offset_y || 0))), n.style.left = e.x + "px", n.style.top = e.y + "px"
            }
    ...
    var n = gantt.env.isIE ? document.documentElement : document.body, e = {x: t.clientX + n.scrollLeft - n.clientLeft,
        y: t.clientY + n.scrollTop - n.clientTop}, a = gantt._get_position(gantt.$task_data);
    return e.x = e.x - a.x, e.y = e.y - a.y, e
    ...
    if (!this._is_tooltip(e) && (t != this._tooltip_id || this._is_task_line(e))) {
        ...
        this._tooltip_id = t;
        var n = this.getTask(t), a = this.templates.tooltip_text(n.start_date, n.end_date, n);
        return a ? void this._show_tooltip(a, this._tooltip_pos(e)) : void this._hide_tooltip()
    }
